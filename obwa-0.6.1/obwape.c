...
int print_bwape_usage(gap_opt_t *opt,pe_opt_t *popt) {
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "Program:  obwape combines \'aln\' and \'sampe\' from bwa (version %s) into one command.\n",PACKAGE_VERSION);
  fprintf(stderr, "Version:  %s\n",BWAPE_VERSION);
  fprintf(stderr, "Contact:  Margus Lukk <margus.lukk@cruk.cam.ac.uk>\n\n");
  fprintf(stderr, "Usage:    obwape [options] <prefix> <in1.fq> <in2.fq>\n");
  fprintf(stderr, "Options:  aln, samse, sampe followed by options as entered in bwa but surrounded by quotes.\n");
  fprintf(stderr, "Examples: obwape hg19.fa in1.fq in2.fq\n\n");
  fprintf(stderr, "          obwape aln \"-e 2\" bwape \"-a 300 -o 2\" hg19.fa in1.fq in2.fq\n\n");
...
int parse_aln(gap_opt_t *opt, char *args) {
  ...
  for(i = 0; i < argc; i++) {
    ...
    switch (c) {
    case 'n':
      if (strstr(argv[i+1], ".")) opt->fnr = atof(argv[i+1]), opt->max_diff = -1;
      else opt->max_diff = atoi(argv[i+1]), opt->fnr = -1.0;
      break;
    case 'o': opt->max_gapo = atoi(argv[i+1]); break;
    case 'e': opte = atoi(argv[i+1]); break;
    case 'M': opt->s_mm = atoi(argv[i+1]); break;
    case 'O': opt->s_gapo = atoi(argv[i+1]); break;
    case 'E': opt->s_gape = atoi(argv[i+1]); break;
    case 'd': opt->max_del_occ = atoi(argv[i+1]); break;
    case 'i': opt->indel_end_skip = atoi(argv[i+1]); break;
    case 'l': opt->seed_len = atoi(argv[i+1]); break;
    case 'k': opt->max_seed_diff = atoi(argv[i+1]); break;
    case 'm': opt->max_entries = atoi(argv[i+1]); break;
    case 't': opt->n_threads = atoi(argv[i+1]); break;
    case 'L': opt->mode |= BWA_MODE_LOGGAP; break;
    case 'R': opt->max_top2 = atoi(argv[i+1]); break;
    case 'q': opt->trim_qual = atoi(argv[i+1]); break;
    ...
    case 'B': opt->mode |= atoi(argv[i+1]) << 24; break;
    default: return 1;
    }
  }
  if (opte > 0) {
...
int parse_sampe(pe_opt_t *popt,char *bwa_rg_line,char *bwa_rg_id, char *args) {
  ...
  for(i = 0; i < argc; i++) {
    ...
    switch (c) {
    case 'r':
      if (bwa_set_rg(argv[i+1]) < 0) {
	fprintf(stderr, "[%s] malformated @RG line\n", __func__);
	return 1;
      }
      break;
    case 'a': popt->max_isize = atoi(argv[i+1]); break;
    case 'o': popt->max_occ = atoi(argv[i+1]); break;
    case 's': popt->is_sw = 0; break;
    case 'P': popt->is_preload = 1; break;
    case 'n': popt->n_multi = atoi(argv[i+1]); break;
    case 'N': popt->N_multi = atoi(argv[i+1]); break;
    case 'c': popt->ap_prior = atof(argv[i+1]); break;
    case 'f': xreopen(argv[i+1], "w", stdout); break;
    case 'A': popt->force_isize = 1; break;
    default: return 1;
    }
  }
...
int obwape_bwa_cal_pac_pos_pe(const bntseq_t *bns, const char *prefix, bwt_t *const _bwt, int n_seqs, bwa_seq_t *seqs[2] , bwa_seq_t *seqsAln[2], isize_info_t *ii, const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii) {
...
int obwape_core(char *prefix, char *fn_fa1, char *fn_fa2, gap_opt_t *opt, pe_opt_t *popt) {
  ...
  if (!(opt->mode & BWA_MODE_COMPREAD)) {
    ...
    // if (popt->is_preload) {
      fprintf(stderr,"[obwape] Loading bwt index ...");
      strcpy(str, prefix); strcat(str, ".bwt"); bwt = bwt_restore_bwt(str);
      strcpy(str, prefix); strcat(str, ".sa"); bwt_restore_sa(str, bwt);
      pac = (ubyte_t*)calloc(bns->l_pac/4+1, 1);
      rewind(bns->fp_pac);
  ...
  while(1) {
    ...
    if(n_seqs != n_seqs2) {
      fprintf(stderr,"[obwape] Different number of reads in files %s and %s\n",fn_fa1, fn_fa2);
      break;
    }
    if(n_seqs == 0) break;

    // take a copy of seqs
    seqsC[0] = obwape_copy_seqs(seqs[0],n_seqs,opt);
    seqsC[1] = obwape_copy_seqs(seqs[1],n_seqs,opt);
    
    // align
    
    t = clock();
    fprintf(stderr, "[bwa_aln_core] calculate SA coordinate 1 ... ");
    obwape_bwa_aln_core_inner(bwt, seqsC[0], n_seqs,opt);
    fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();

    fprintf(stderr, "[bwa_aln_core] calculate SA coordinate 2 ... ");
    obwape_bwa_aln_core_inner(bwt, seqsC[1], n_seqs,opt);
    fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();

    fprintf(stderr, "[bwa_sai2sam_pe_core] convert to sequence coordinate... \n");
    cnt_chg = obwape_bwa_cal_pac_pos_pe(bns, prefix, bwt, n_seqs, seqs, seqsC, &ii, popt, opt, &last_ii);
    fprintf(stderr, "[bwa_sai2sam_pe_core] time elapses: %.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
    fprintf(stderr, "[bwa_sai2sam_pe_core] changing coordinates of %d alignments.\n", cnt_chg);
...
int obwape(int argc, char *argv[]) {
  ...
  // aln + sampe
  obwape_core(argv[j],argv[j+1],argv[j+2],opt,popt);
